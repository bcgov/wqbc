...
test_that("identify_outliers", {
  ...
  expect_message(identify_outliers(wqbc::dummy, messages = TRUE), "Identified 2 outliers in water quality data.")
  ...
  expect_message(identify_outliers(data, threshold = 10, ignore_undetected = FALSE, messages = TRUE), "Identified 3 outliers in water quality data.")
  data <- data.frame(Value = c(-101, rep(100, 2), rep(0, 1000)))
  expect_message(identify_outliers(data, threshold = 10, ignore_undetected = FALSE, messages = TRUE), "Identified 2 outliers in water quality data.")
  expect_message(identify_outliers(data, threshold = 10, ignore_undetected = FALSE, large_only = FALSE, messages = TRUE), "Identified 3 outliers in water quality data.")

  data <- expand.grid(Value = c(-101, rep(100, 2), rep(0, 1000)), Other = c("x", "y"))
  expect_message(identify_outliers(data, by = "Other", threshold = 10, ignore_undetected = FALSE, large_only = FALSE, messages = TRUE), "Identified 6 outliers in water quality data.")
})
