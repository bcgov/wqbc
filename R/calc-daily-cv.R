calc_daily_cv_variable_date <- function (x, messages) {
  if(length(unique(x$Units)) != 1) {
    if(messages)
      message("inconsistent units for ", x$Variable[1], " on ", x$Date[1])
    x$Value <- convert_units(x$Value, from = x$Units, to = x$Units[1])
    x$Units <- x$Units[1]
  }
...
calc_daily_cv_variable <- function (x, messages) {
  if(anyDuplicated(x$Date))
    x <- plyr::ddply(x, "Date", calc_daily_cv_variable_date, messages)
  x
}

#' Calculates Daily Coefficient of Variation
#'
#' Calculates the daily coefficient of variation for values for
...
#' @examples
#'
#' data <- data.frame(Variable = "A Variable", Date = Sys.Date(),
#'                    Value = c(20, 1, 2), Units = c("mg/L", "mg/L", "mg/L"))
#'
#' data <- rbind(data, data.frame(Variable = "Another Variable", Date = Sys.Date(),
#'                    Value = c(1, 10^3), Units = c("mg/L", "ug/L")))
#'
#' calc_daily_cv(data, messages = TRUE)
#' @export
calc_daily_cv <- function (x, messages = getOption("wqbc.messages", default = TRUE),
  ...
  plyr::ddply(x, .variables = "Variable", .fun = calc_daily_cv_variable,
