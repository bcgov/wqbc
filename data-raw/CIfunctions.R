...
AllInd <- function(DtObj, i = 1:nrow(DtObj)) {
  ...
  # We find F1, F2, F3 and WQI. Here DtObj is the ratio of Data and
  # Objective

  # matrix. We do so in order to avoid duplicated computation in
  # bootstrapping.
...
BootIndex <- function(DataByObj, R = 10000) {
  ...
  # This function finds the estimates in column-wise bootstraping
  # procedure.
  ...
  for (j in 1:R) {
    ...
    if (NFC > 0) {
      ...
      SampleInd <- apply(matrix(rep(N, NFC), nrow = 1), 2,
                         function(n) sample(1:n, n, replace = TRUE))

      for (k in 1:NFC) {
        BootData[, FailCol[k]] <- DataByObj[SampleInd[, k], FailCol[k]]
      }
