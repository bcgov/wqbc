...
The data used in the examples are from the fraser river basin and an example taken from the CCME WQI [User's Manual](http://www.ccme.ca/files/Resources/calculators/WQI%20User%27s%20Manual%20%28en%29.pdf "download pdf").  Additional data is available from a sister package `wqdata`.  This package is to serve as a repository for a number of datasets on water quality monitoring in Brittish Columbia.

The following methods of analysing water quality are provided:

1. Calcultion of compliance thresholds for water quality variables
2. Calculation of the CCME water qualty index
3. Methods for visualisation of water quality indices
...
This function will be discussed further in the following section, but note for now that the variable listed as `DO` in the the `ccme` data does not have a corresponding limit defined in `lookup_limits` and so for water quality indices to be calculated for the `ccme` data, there must be compliance thresholds provided for each observation. This should be included by providing either upper limits in a column called `UpperLimit`, or lower limits in `LowerLimit`, for example
...
And as expectd by setting the compliance thresholds arbitrarily high, we get no failures and an Excellent marking!
...
The function `calc_wqi` performs and number of tasks.  The first of these is to check if the dataset contains user defined limits in columns called `UpperLimit` or `LowerLimit`.  If the dataset has these columns, the function proceeds directly to calculating the water quality index.  

If the data do not contain user defined limits, as is the case for the `fraser` datset, the next steps are to check and standardise the data so that they can be matched with known water quality limits. The list of known water quality limits can be queried using the function `lookup_limits`, which has arguments to help evaluate the limit.  Because some limits are dependent on the concentrations of other chemicals, `lookup_limits` has the following arguments: `ph`, `hardness`, `chloride` and `methyl_mercury`.  In addition because limits are different depending on the time scale, there is a further argument `term` which can take the values "short" or "long". 

```{r}
lookup_limits(ph = 7)
```

From this we can see that there are 23 standard variables with limits defined.  Some variable in the previous example have `NA` because these limits require knowledge of hardness, choloride or methyl mercury concentrations.
...
Now that we have observations for a range of standard variables it is possible to calculate the relavent limits with which to assess compliance.  Taking into account the various rules for calculating compliance thresholds for the standard variables, the function `calc_limits` calculates the limits required to evaluate the water quality index.  Again, if it is required to calculate the water quality index for each site, then the `by` argument is used

```{r}
calc_limits(data2012, by = "SiteID", term = "long")
```

Now the 2012 Fraser river basin data has been reduced to daily means of the standard variables, and compliance thresholds have been attributed to each observation.  A visual inspection of the data shows that for the period starting the 11th April, several variables at the `BC08MC0001` site are above their long limits, i.e. Cobalt Total, Copper Total, Silver and Zinc Total.
...
The to attribure these grouped water quality indices back to the individual sites, a basic R function called `merge` is used, which merges based on column names.  An additional trick is used here where the function `unique` keeps only the unique combinations of `NorthSouth`, `SiteID`, `Lat` and `Long`.
...
This example calculates short term water quality indices over multiple years (in this case from 2002 to 2012), and by site.  The function `facet_wrap` is then used to produce a grided plot output showing multiple years

```{r, eval = FALSE}
options(wqbc.messages = FALSE)
data02to12 <- subset(fraser, year(Date) %in% 2002:2012)
data02to12 $ year <- year(data02to12 $ Date)
limits02to12 <- calc_limits(data02to12, by = c("year", "SiteID"), term = "short") 
wqi02to12 <- calc_wqi(limits02to12, by = c("year", "SiteID")) 
```
To visualise the water quality indices over the years, a handy function from the `ggpot2` package called `facet_wrap` can be used.  This is because the ploting functions return a ggplot2 object which can be reploted and adapted - see the [ggplot2 webpage](http://ggplot2.org/) for lots of helpful ideas on how to add to and customise these type of plots.

```{r, fig.width = 7, fig.height = 7, dpi = 100, eval = FALSE}
p <- plot_map_wqis(wqi02to12) 
p + facet_wrap(~year)
```
